...
SPIInstance *SPIRegister(const SPI_Init_Config_s *conf)
{
    if (idx >= MX_SPI_BUS_SLAVE_CNT) // 超过最大实例数
        while (1)
            LOGERROR("SPI exceeds the maximum number of instances when SPIRegister");
    ...
    else if (instance->spi_handle->Instance == SPI3)
    {
        instance->cs_pin_state = &SPIDeviceOnGoing[1];;
    }
    else
    {
        while (1)
            LOGERROR("SPI writes to a non-existent port when SPIRegister");
    }
    spi_instance[idx++] = instance;
    return instance;
}

void SPITransmit(const SPIInstance *spi_ins, const uint8_t *ptr_data, const uint8_t len)
{
    // 拉低片选,开始传输(选中从机)
    ...
    default:
        while (1)
        {
            LOGERROR("SPI sets an error mode when SPITransmit");// error mode! 请查看是否正确设置模式，或出现指针越界导致模式被异常修改的情况
        }
    }
}
...
void SPIRecv(SPIInstance *spi_ins, uint8_t *ptr_data, uint8_t len)
{
    ...
    default:
        while (1)
        {
            LOGERROR("SPI sets an error mode when SPIRecv");// error mode! 请查看是否正确设置模式，或出现指针越界导致模式被异常修改的情况
        }
    }
}

void SPITransRecv(SPIInstance *spi_ins, uint8_t *ptr_data_rx, const uint8_t *ptr_data_tx, const uint8_t len)
{
    ...
    *spi_ins->cs_pin_state =
    ...
    default:
        while (1)
        {
            LOGERROR("SPI sets an error mode when SPITransRecv");// error mode! 请查看是否正确设置模式，或出现指针越界导致模式被异常修改的情况
        }
    }
}
...
void SPISetMode(SPIInstance *spi_ins, SPI_TXRX_MODE_e spi_mode)
{
    if (spi_mode != SPI_DMA_MODE && spi_mode != SPI_IT_MODE && spi_mode != SPI_BLOCK_MODE)
        while (1)
        {
            LOGERROR("SPI sets an error mode when SPISetMode");
 ...
 * @param hspi spi handle
 */
void HAL_SPI_TxRxCpltCallback(const SPI_HandleTypeDef *hspi)
{
